...
Reader::Reader(const path& dir_path, Connection& fls) {
	// read footer
	{ m_footer = make_footer(dir_path / FOOTER_FILE_NAME); }
	...
	// init level 1 expression
	{
		...
		for (n_t col_idx {0}; col_idx < m_footer->size(); ++col_idx) {
			...
			n_operators.push_back(ExprExecutor::CountOperator(*physical_expr));
...
vector<sp<PhysicalExpr>>& Reader::get_chunk(const n_t vec_idx) {
	for (n_t col_idx {0}; col_idx < m_footer->size(); ++col_idx) {
		...
		ExprExecutor::execute(physical_expr, n_operators[col_idx], vec_idx);
	}
...
Footer& Reader::footer() const { return *m_footer; }
